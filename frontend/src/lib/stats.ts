...
export function getAggregatedData<T extends TimestampedItem>(
    ...
    let start: Date;
    let end = now;
    let explicitStartKey: string | null = null;
    let explicitEndKey: string | null = null;

    if (explicitDateRange && explicitDateRange.start && explicitDateRange.end) {
        explicitStartKey = explicitDateRange.start;
        explicitEndKey = explicitDateRange.end;
    ...
    if (granularity === 'hour' && explicitStartKey && explicitEndKey) {
        let cursor = parseExplicitHourCursor(explicitStartKey);
        const endCursor = parseExplicitHourCursor(explicitEndKey);
        ...
        while (cursor <= endCursor) {
            ...
            const key = formatExplicitHourKey(cursor);
            dataMap[key] = {
                date: key,
                count: 0,
                label: `${bucketDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric', timeZone: 'UTC' })}, ${String(cursor.getUTCHours()).padStart(2, '0')}:00`,
                fullDate: bucketDate.toISOString(),
            };
...
function parseExplicitHourCursor(key: string): Date {
    const [datePart, hourPart] = key.split('T');
    const [year, month, day] = datePart.split('-').map(Number);
    return new Date(Date.UTC(year, month - 1, day, Number(hourPart), 0, 0, 0));
}

function formatExplicitHourKey(date: Date): string {
